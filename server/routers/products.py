...
match_scan_model = ns_products.model("MatchScanRequest", {
    "record_id": fields.Integer(required=True, description="Target camera scan record ID")
})
...
class ProductCount(Resource):
    ...
    @ns_products.param("category_id", "Filter by category ID", type=int)
...
class ProductDetail(Resource):
    ...
    @ns_products.response(404, "Product not found")
    @admin_required
    def put(self, product_id: int, current_user: dict):
        data = request.json or {}
        with get_db_session() as session:
            product = session.get(Product, product_id)
            if not product:
    ...
    @ns_products.response(404, "Product not found")
    @admin_required
    def delete(self, product_id: int, current_user: dict):
        with get_db_session() as session:
            product = session.get(Product, product_id)
            if not product:
                return {"detail": f"Product ID {product_id} not found."}, 404
