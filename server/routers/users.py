...
user_patch_model = ns_users.model("UserPatchRequest", {
    "email": fields.String(description="Email address"),
    "full_name": fields.String(description="Full name"),
    "avatar_url": fields.String(description="Avatar URL"),
    "is_active": fields.Boolean(description="Account active state"),
...
user_status_patch_model = ns_users.model("UserStatusPatchRequest", {
    "is_active": fields.Boolean(required=True, description="Account active status")
})

user_stats_model = ns_users.model("UserStatsResponse", {
    "total_users": fields.Integer,
    "active_users": fields.Integer,
    "inactive_users": fields.Integer,
    "admin_users": fields.Integer,
    "standard_users": fields.Integer
})
...
@ns_users.route("/stats")
class UserStats(Resource):
    @ns_users.doc("get_user_stats", security="Bearer", description="Aggregated user account statistics")
    @ns_users.response(200, "Success", user_stats_model)
    @admin_required
    def get(self, current_user: dict):
        with get_db_session() as session:
            ...
            admins = sum(1 for u in users if u.role_id == 1)
            standard = sum(1 for u in users if u.role_id != 1)
            return {
                "total_users": total,
                "active_users": active,
...
class UserDetail(Resource):
    ...
    @ns_users.response(404, "User not found")
    @admin_required
    def put(self, user_id: str, current_user: dict):
        data = request.json or {}
        with get_db_session() as session:
            user = session.get(User, user_id)
            if not user:
    ...
    @ns_users.doc("patch_user", security="Bearer", description="Partially update user attributes")
    @ns_users.expect(user_patch_model, validate=False)
    @ns_users.response(200, "User updated", user_model)
    @ns_users.response(404, "User not found")
    @admin_required
    def patch(self, user_id: str, current_user: dict):
        data = request.json or {}
        with get_db_session() as session:
            user = session.get(User, user_id)
            if not user:
    ...
    @ns_users.response(404, "User not found")
    @admin_required
    def delete(self, user_id: str, current_user: dict):
...
@ns_users.route("/<string:user_id>/status")
class UserStatusResource(Resource):
    @ns_users.doc("patch_user_status", security="Bearer", description="Toggle or update user active status")
    @ns_users.expect(user_status_patch_model, validate=True)
    @ns_users.response(200, "Status updated", user_model)
    @ns_users.response(404, "User not found")
    @admin_required
    def patch(self, user_id: str, current_user: dict):
        data = request.json or {}
...
@ns_users.route("/me")
class UserSelfProfile(Resource):
    @ns_users.doc("get_my_user_profile", security="Bearer", description="Get own user profile")
    @ns_users.response(200, "Success", user_model)
    @token_required
    def get(self, current_user: dict):
        with get_db_session() as session:
    ...
    @ns_users.doc("patch_my_user_profile", security="Bearer", description="Update own user profile")
    @ns_users.expect(user_patch_model, validate=False)
    @ns_users.response(200, "Profile updated", user_model)
    @token_required
