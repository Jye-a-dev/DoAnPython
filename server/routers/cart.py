...
cart_add_model = ns_cart.model("CartAddRequest", {
    ...
    "record_id": fields.Integer(required=False, description="Scan record ID where item was detected")
})
...
@ns_cart.route("/items/<int:item_id>")
class CartItemResource(Resource):
    ...
    @token_required
    def put(self, item_id: int, current_user: dict):
        data = request.json or {}
    ...
    @ns_cart.response(404, "Cart item not found")
    @token_required
    def delete(self, item_id: int, current_user: dict):
        with get_db_session() as session:
            item = session.get(CartItem, item_id)
            if not item or item.user_id != current_user["id"]:
                return {"detail": f"Không tìm thấy món hàng ID {item_id} trong giỏ."}, 404
